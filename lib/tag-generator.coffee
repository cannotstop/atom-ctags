...
module.exports = (codepath, isAppend, cmdArgs, callback)->
  ...
  jarPath = path.resolve(__dirname, '..', 'vendor', "test.jar")
  outputPath = __dirname.toString().replace "packages\\atom-ctags\\lib", "packages\\language-tcl\\snippets\\MediaSuite-tcl.cson"
  apiPath = "D:\\document\\111.30\\Automation\\trunk\\TCLFramework\\lib\\suts\\cs\\spi"
  #apiPath = codepath
  jarOption = "java -jar " + jarPath + " -apiPath "+ apiPath + " -fileName "+ outputPath
  console.log jarOption
  flag = (codepath.indexOf '.tcl') * (codepath.indexOf '.xotcl')
  if flag > 0
    exec jarOption, (error, stdout, stderr)->
      console.log('stdout: ' + stdout) if stdout
      console.log('stderr: ' + stderr) if stderr
      console.log('exec error: ' + error) if error
      console.log "Snippet over"
  else
    console.log "Skip"
